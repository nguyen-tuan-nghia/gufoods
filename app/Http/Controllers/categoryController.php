...
class categoryController extends Controller
{
    public function locgia(Request $request, $id)
    {
        ...
        $product = product::with('gallery')->whereBetween('price', [$request->min, $request->max])->where('status', '!=', 0)->where(function($query) use($category){
            $query->where('category_id','LIKE','%'.$category->id.'%')->orWhere('category_id','LIKE','%, '.$category->id.'%');
        })
        ->get();
        if(count($product)==0){
            return 0;
    ...
    public function danhmuc($slug)
    {
        ...
        $product = product::with('gallery')->where('status', '!=', 0)->where(function($query) use($category){
            $query->where('category_id','LIKE','%'.$category->id.'%')->orWhere('category_id','LIKE','%, '.$category->id.'%');
        })->get();
    ...
    public function delete($id)
    {
        ...
        $product = product::where('category_id','LIKE','%'.$id.', %')->orWhere('category_id','LIKE','%, '.$id.'%')->get();
        if (count($product) == 0) {
            if ($category) {
